...
# ==============================================================================
set.seed(42)
N_sims     <- 100  # Configured for a solid ~2.5 to 3 hour production run
N_patients <- 20
T_max      <- 28
target_dlt <- 0.25
...
run_single_trial_simulation <- function(sim_id, compiled_stan_model, N_patients, T_max, target_dlt, q_skeleton, logit_q, true_alpha, true_b_sex, true_b_bmi) {
  ...
  cat(sprintf("[%s] Worker node engaged: Starting Simulation Run #%d / 100\n", Sys.time(), sim_id), 
...
# ==============================================================================
num_workers <- 6
cat(sprintf("\nLaunching parallel cluster across %d workers for 100 production runs...\n", num_workers))
...
# Reviewer-Suggested Improvement: Fixed Reference-Dose Risk Alignment Correlation
all_baseline_risks <- unlist(lapply(raw_parallel_results, function(x) {
  sapply(1:N_patients, function(idx) {
    # Anchoring baseline risk cleanly at a population reference level (Dose 3)
    get_true_risk(3, x$demographics$female[idx], x$demographics$bmi[idx] - 26, true_alpha, true_b_sex, true_b_bmi)
  })
}))
risk_alignment_r <- cor(all_baseline_risks, all_assigned_doses)
...
cat("\n========================================================================\n")
cat("          UPGRADED CLINICAL METRICS (PRODUCTION 100 SCORECARD)          \n")
cat("========================================================================\n")
...
if (nrow(fragile_cohort) > 0) {
  ...
  cat("Fragile Cohort note: No low-BMI females happened to be sampled in this run.\n")
}
