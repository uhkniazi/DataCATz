...
stanDso = rstan::stan_model(file='BayesRegression/linearRegressionRandomEffects.stan')

m = model.matrix(resp ~ pred, data=dfData)

lStanData = list(Ntotal=nrow(dfData), Nclusters=nlevels(dfData$group), 
                 NgroupMap=dfData$groupIndex, Ncol=ncol(m), X=m,
                 y=dfData$resp)

fit.stan = sampling(stanDso, data=lStanData, iter=5000, chains=4, pars=c('betas', 'sigmaRan', 'sigmaPop', 'rGroupsJitter'),
                    cores=4)
