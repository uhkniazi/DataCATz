...
fit.2 = glm(use ~ urban, data=Contraception, family = binomial(link='logit'))
summary(fit.2)
...
m1 = model.matrix(use ~ 1 + children, data=Contraception)
m2 = model.matrix(use ~ 1 + urban, data=Contraception)
m1 = as.matrix(m1[,-1])
m2 = as.matrix(m2[,-1])
## prepare data
lStanData = list(Ntotal=nrow(Contraception), Ncol1=1, Ncol2=1,
                 X1=m1, X2=m2, iMixtures=2, y=ifelse(Contraception$use == 'N', 0, 1))

fit.stan = sampling(stanDso, data=lStanData, iter=2000, chains=1, cores=1)
