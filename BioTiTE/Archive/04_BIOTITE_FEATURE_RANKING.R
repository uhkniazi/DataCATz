# ==============================================================================
# File: 04_BIOTITE_FEATURE_RANKING.R
...
#       (Ridge, Random Forest, Standard Stan, BioTiTE V3) on centered data.
# ==============================================================================
...
# --- 4.2 Tier 0: Frequentist Continuous Models (Ridge & Random Forest) ---
...
# Including dose factor alongside biomarkers so RF solves DLT ~ dose + biomarkers
rf_input_df <- df_sim[, c("dose", paste0("X_", 1:100))]
fit_rf      <- randomForest(x = rf_input_df, y = factor(df_sim$dlt), importance = TRUE, ntree = 500)
rf_importance <- importance(fit_rf, type = 1)[, 1] # %IncMSE / MeanDecreaseAccuracy
...
auc_summary <- data.frame(
  Model_Class = c("Frequentist Ridge (L2)", "Random Forest (%IncMSE)", "Standard Stan GLM", "BioTiTE V3"),
  Biomarker_AUC = c(
