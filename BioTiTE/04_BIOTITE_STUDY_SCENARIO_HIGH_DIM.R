...
#       dose-confounding adjustment, and posterior extraction for BioTiTE.
# ==============================================================================
...
generate_highdim_scenario_data <- function(n = 40, 
  ...
  # 5 Causal signals, 95 Null signals
  ...
  # --- Construct Covariate Matrix (Continuous & Binary Mix) ---
  X_mat <- matrix(0, nrow = n, ncol = n_features)
  colnames(X_mat) <- paste0("X_", 1:n_features)
  
  # Feature 1 (Causal): Binary & Dose-Confounded (prevalence increases with dose)
  g1_prob <- c(0.10, 0.20, 0.40, 0.60, 0.80)
  X_mat[, 1] <- rbinom(n, size = 1, prob = g1_prob[dose_raw])
  
  # Feature 2 (Causal): Continuous & Dose-Confounded (mean expression scales with dose)
  X_mat[, 2] <- rnorm(n, mean = as.numeric(scale(dose_raw)), sd = 1.0)
  
  # Features 3-5 (Causal): Unconfounded signals (mix of continuous & binary)
  X_mat[, 3] <- rnorm(n, mean = 0, sd = 1.0)
  X_mat[, 4] <- rbinom(n, size = 1, prob = 0.35)
  X_mat[, 5] <- rnorm(n, mean = 0, sd = 1.0)
  
  # Features 6-100 (Null Noise): 50% Continuous Gaussian, 50% Binary Bernoulli
  ...
  # Linear predictor and DLT Bernoulli draws
  ...
  # Design matrix: 5 dose indicators + 100 features = 105 columns
...
# Calculate feature ranks based on absolute effect magnitude (100 = top ranked feature)
glm_ranks <- rank(abs(glm_beta))
std_ranks <- rank(abs(std_beta))
bio_ranks <- rank(abs(bio_beta))

# Extract ranks of the 5 true causal features
...
# Calculate global ROC-AUC for separating 5 causal signals from 95 null signals
...
# ==============================================================================
# Mean and SD of posterior dose intercepts (alpha) to use as prior in Trial 2
alpha_post_means <- apply(ext_bio$alpha, 2, mean)
alpha_post_sds   <- apply(ext_bio$alpha, 2, sd)

cat("\n======================================================================\n")
cat("EXTRACTED DOSE INTERCEPT POSTERIORS (To pass as priors into Trial 2)\n")
cat("======================================================================\n")
