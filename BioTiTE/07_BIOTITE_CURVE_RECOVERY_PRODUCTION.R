...
run_scenario2_production <- function(N_sims = 500, N_patients = 40) {
  ...
  # Compute Paired Wilcoxon Tests: BioTiTE P=100 vs Standard Stan
  wilcox_std_res <- wilcox.test(rmse_std_vec, rmse_bio_cov_v, paired = TRUE)
  win_rate_std   <- mean(rmse_bio_cov_v < rmse_std_vec) * 100
  
  # Compute Paired Wilcoxon Tests: De-confounding Benefit (BioTiTE P=100 vs BioTiTE P=0)
  wilcox_cov_effect <- wilcox.test(rmse_bio_nocov_v, rmse_bio_cov_v, paired = TRUE)
  win_rate_cov      <- mean(rmse_bio_cov_v < rmse_bio_nocov_v) * 100
  ...
  cat("======================================================================\n")
  cat("PAIRED COMPARISON DIAGNOSTICS\n")
  cat("======================================================================\n")
  cat("--- 1. BioTiTE (P=100) vs Standard Stan GLM (P=100) ---\n")
  cat(sprintf("BioTiTE Lower RMSE Win Rate: %.1f%% of simulations\n", win_rate_std))
  cat(sprintf("Paired Wilcoxon Signed-Rank Test p-value: %e\n\n", wilcox_std_res$p.value))
  
  cat("--- 2. De-confounding Benefit: BioTiTE (P=100) vs BioTiTE (P=0) ---\n")
  cat(sprintf("Biomarker Adjustment Win Rate: %.1f%% of simulations\n", win_rate_cov))
  cat(sprintf("Paired Wilcoxon p-value (P=100 vs P=0): %e\n\n", wilcox_cov_effect$p.value))
