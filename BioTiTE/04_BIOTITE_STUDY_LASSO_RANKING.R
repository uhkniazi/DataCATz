# ==============================================================================
# File: 04_BIOTITE_STUDY_LASSO_RANKING.R
# Auth: u.niazi@soton.ac.uk
# Date: 10/09/2026
# Desc: High-dimensional ($P=100$) feature prioritization script comparing 
#       LASSO (Tier 0), Standard Stan (Tier 1), and BioTiTE V3 (Tier 2) 
#       using centered binary {-0.5, +0.5} and continuous covariates.
# ==============================================================================
...
# ==============================================================================
# --- Tier 0: LASSO via cv.glmnet ---
# penalty.factor = 0 for cols 1:5 allows dose baselines to remain unpenalized,
# while L1 penalty is applied to cols 6:105 (genomic features X_1 to X_100).
...
lasso_beta <- rep(0, 100)
names(lasso_beta) <- paste0("X_", 1:100)
matched_names <- intersect(names(lasso_beta), rownames(lasso_coef_mat))
lasso_beta[matched_names] <- lasso_coef_mat[matched_names, 1]
...
lasso_ranks <- rank(abs(lasso_beta))
...
rank_summary <- data.frame(
  ...
  LASSO_Rank    = lasso_ranks[1:5],
  StdStan_Rank  = std_ranks[1:5],
  BioTiTE_Rank  = bio_ranks[1:5]
)

auc_summary <- data.frame(
  Model = c("LASSO (Tier 0)", "Standard Stan (Tier 1)", "BioTiTE V3 (Tier 2)"),
  Biomarker_AUC = c(
    calc_feature_auc(abs(lasso_beta), causal_mask),
