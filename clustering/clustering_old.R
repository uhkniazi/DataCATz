...
######################################################################################
## extract components 
pr.dna = prcomp(mData.dna, scale=T, center = T)
pr.rna = prcomp(mData.rna, scale=T, center = T)

plot(pr.dna$sdev)
plot(pr.rna$sdev)

## take the 6 components from both data sets
c1 = pr.dna$x#[,1:6]
c2 = pr.rna$x#[,1:6]

mData = cbind(c1, c2)
mData = scale(mData)
pr.comb = prcomp(mData)
plot(pr.comb$x, col=ivCols, pch=20, main='Analytical')
text(pr.comb$x, labels = rownames(mData), adj = 1, cex = 0.6)
plot(pr.comb$sdev)
...
#mData = mData.dna
mData = cbind(mData.dna, mData.rna)
...
## set stan data
lStanData = list(Ntotal=nrow(mData), Nvars=ncol(mData), Neigens=41,
                 y=(mData))

pr.out = prcomp(mData, scale=F, center = F)
plot(pr.out$x, col=ivCols, pch=20, main='Analytical')
text(pr.out$x, labels = rownames(mData), adj = 1, cex = 0.6)
plot(pr.out$sdev)

fit.stan2 = sampling(stanDso2, data=lStanData, iter=20000, chains=3, cores=3)#, #init=initf, 
#control=list(adapt_delta=0.99, max_treedepth = 10)) # some additional control values/not necessary usually
...
dim(mComp)
mComp.all = lapply(seq_along(1:ncol(mComp)), function(x){
...
dim(d2)
d2 = d2[nrow(d2):1,]
image(1:ncol(d2), 1:ncol(d2), d2, axes=F, xlab='', ylab='', col=heat.colors(5))
axis(1, 1:ncol(d2), colnames(d2), cex.axis=0.5, las=3)
axis(2, 1:ncol(d2), rownames(d2), cex.axis=0.4, las=1)
