...
get_true_optimal_dose <- function(female, bmi_centered, true_alpha, true_b_sex, true_b_bmi, target_dlt = 0.25) {
  true_risks <- sapply(1:5, function(d) get_true_risk(d, female, bmi_centered, true_alpha, true_b_sex, true_b_bmi))
...
# ==============================================================================
# CORRECTED SIMULATION LOOP (Tracks fitted curves at Patient 20)
# ==============================================================================
run_single_curve_simulation <- function(sim_id, compiled_stan_model, N_patients, T_max, target_dlt, q_skeleton, logit_q, true_alpha, true_b_sex, true_b_bmi) {
  ...
  # Re-fit trialr one last time to capture the final full-trial curve
  ...
  X_cov <- model.matrix(~ 0 + factor(d_m5cov, levels=1:5) + female + bmi_centered, data=patients)
  ...
  # Extract curves across clinical phenotypes
  curve_m5cov_base      <- sapply(1:5, function(d) mean(plogis(ext$alpha[, d] + ext$beta_cov[,1]*0 + ext$beta_cov[,2]*0)))
  curve_m5cov_fragile   <- sapply(1:5, function(d) mean(plogis(ext$alpha[, d] + ext$beta_cov[,1]*1 + ext$beta_cov[,2]*(-5))))
  curve_m5cov_resilient <- sapply(1:5, function(d) mean(plogis(ext$alpha[, d] + ext$beta_cov[,1]*0 + ext$beta_cov[,2]*5)))
...
# Calculate True Theoretical Curves for Comparison Profiles
true_pop       <- sapply(1:5, function(d) get_true_risk(d, 0, 0, true_alpha, true_b_sex, true_b_bmi))
true_fragile   <- sapply(1:5, function(d) get_true_risk(d, 1, -5, true_alpha, true_b_sex, true_b_bmi))
true_resilient <- sapply(1:5, function(d) get_true_risk(d, 0, 5, true_alpha, true_b_sex, true_b_bmi))
...
# mfrow = c(1, 1) ensures each high-level plot() automatically creates a new page
...
# FIXED: Label updated to reflect specific reference profile definition instead of global baseline
legend("topleft", 
       legend = c("Initial Skeleton Input", "True Population Reality", 
                  "trialr (TiTE-CRM) Mean Fit", "M5Cov Baseline Fit (Male, Mean BMI)"),
...
dev.off()
cat("Successfully generated two-page vector graphics file: 'curves.pdf'\n")