# Name: tite_crm_simulation_C.R
# Auth: u.niazi@soton.ac.uk
# Date: 23/06/2026
# Desc: TiTE-CRM Models used in Phase 1 clinical trials to assess dosage based
#       on toxicity skeleton. Simulation for Scenario C (The Ultimate Test: 
#       Severe Skeleton Misspecification + High Covariate Heterogeneity)
...
# Note: The skeleton assumes Dose 3 is the MTD (0.25)
q_skeleton <- c(0.05, 0.12, 0.25, 0.40, 0.55)
logit_q    <- qlogis(q_skeleton) # Convert skeleton risks to logit scale for Stan priors

# --- BIOLOGICAL TRUTH PARAMETERS (Scenario C: Severe Misspecification + Covariates) ---
# [Crucial Change] True curve is significantly steeper than what the skeleton assumes!
true_alpha <- qlogis(c(0.02, 0.15, 0.35, 0.55, 0.70)) 
true_b_sex <- 0.8   # Females face higher underlying risk of toxicity (+0.8 log-odds)
true_b_bmi <- -0.3  # Lower BMI values dramatically increase toxicity risk (-0.3 per unit)
...
run_single_trial_simulation <- function(sim_id, compiled_stan_model, N_patients, T_max, target_dlt, q_skeleton, logit_q, true_alpha, true_b_sex, true_b_bmi) {
  ...
  # Under Scenario C, this will vary wildly because the steep true curve and active coefficients intersect dynamically.
  ...
  for (i in 1:N_patients) {
    ...
    if (i == 1) { d_m5cov[i] <- 1 } else {
      ...
      # In Scenario C, the custom model should flexible bend to adapt to the un-modeled steepness.
...
cat(sprintf("\nLaunching parallel cluster across %d workers for Scenario C...\n", num_workers))
...
cat("\n========================================================================\n")
cat("          PRODUCTION SCORECARD: SCENARIO C EVALUATION (MISSPECIFIED)    \n")
cat("========================================================================\n")
...
oc_summary <- data.frame(
  ...
  # Global MTD Accuracy measures how often the last assigned dose hits the baseline biological optimum.
  # Under Scenario C's steep curve, Dose 2 (0.15 risk) is mathematically chosen over Dose 3 (0.35 risk) 
  # by standard distance minimization for a baseline patient.
  # Note: Left as NA for M5Cov because a personalized model completely bypasses population targets.
  Global_MTD_Accuracy   = c(mean(final_mtd_dfcrm == 2), mean(final_mtd_trialr == 2), mean(final_mtd_m5base == 2), NA) * 100,
